...
class HomeScreenClip extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    var percent2 = 0.68;
    var value = 25;
    var controlPoint1End =
        Offset((size.width + (value / 2)) * percent, height * percent);
    ...
    var leftBottomEnd =
        Offset(width * (1 - percent), (height * percent) - value);
    ...
    path.cubicTo(
        controlPoint1Angle.dx,
        controlPoint1Angle.dy,
        controlPoint1Angle2.dx,
        controlPoint1Angle2.dy,
        controlPoint1End.dx,
        controlPoint1End.dy);
    path.lineTo(leftBottomControl.dx + value, leftBottomControl.dy);
    path.quadraticBezierTo(leftBottomControl.dx, leftBottomControl.dy,
        leftBottomEnd.dx, leftBottomEnd.dy);
    // path.lineTo(controlPoint2.dx, controlPoint2.dy + value);
    path.cubicTo(
        controlPoint2Angle.dx,
        controlPoint2Angle.dy,
        controlPoint2Angle2.dx,
        controlPoint2Angle2.dy,
        controlPoint2End.dx,
        controlPoint2End.dy);
    path.close();
    return path;
